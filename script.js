...
window.bossesDB = {
    ...
    50: { name: "Dragão Negro Primordial", baseHp: 1200, baseAtk: 130, baseXp: 0, baseGold: 5000, icon: "🐉" }
};
...
window.player = {
    ...
    equip(index) {
        ...
        window.ui.log(`Você equipou <b>${item.name}</b>.`, "skill");
    ...
    usePotion() {
        if (this.potions <= 0) return window.ui.log("❌ Sem poções na bolsa!", "dmg-taken");
        if (this.hp === this.maxHp && this.mp === this.maxMp) return window.ui.log("Sua Vida e Mana já estão no máximo.");
        this.potions--; 
        this.heal(60); 
        this.restoreMp(40);
    ...
    buyPotion() {
        if (this.gold >= 20) { this.gold -= 20; this.potions++; window.ui.log("🛒 Comprou 1 Poção Curativa.", "loot"); window.ui.update(); window.game.saveGame(); } 
    ...
    gainXp(amount) {
        ...
        if (this.xp >= this.xpToNext) {
            ...
            this.recalculateStats();
            window.ui.log(`🎊 NÍVEL UP! Você alcançou o Nível ${this.lvl}! (+Atributos)`, "loot");
        }
    }
};

// ==========================================
// EXPLORAÇÃO E SISTEMA
// ==========================================
window.game = {
    state: 'menu',
    ...
    saveGame() {
        ...
        window.ui.log("💾 O seu progresso foi salvo nos pergaminhos.", "sys");
    },

    loadGame() {
        const savedData = localStorage.getItem('aethelgard_save');
        if (savedData) {
    ...
    resetGame() {
        if(confirm("Tem certeza absoluta? O seu Herói será apagado para sempre!")) {
    ...
    chooseClass(classKey) {
        ...
        this.state = 'explore';
        window.ui.log(`Bem-vindo, <b>${cls.name}</b>. A aventura em Aethelgard começa!`, "loot");
        this.saveGame();
    },
    ...
    explore() {
        ...
        if (currentDecade >= 10 && window.player.highestBossDefeated < currentDecade) {
            window.ui.log(`🌩️ Os céus escurecem... Um CHEFÃO se aproxima!`, "dmg-taken");
        ...
        if (roll < 0.45) { 
            ...
            window.ui.log(`🌲 Escondido na folhagem, achou ${foundGold}💰.`, "loot");
            window.ui.update();
            this.saveGame();
        } 
        else if (roll < 0.85) { 
            ...
            window.ui.log(`⛺ Você encontra <b>${npc.name}</b> acampando.`, "sys");
            npc.encounter();
            window.ui.update();
            this.saveGame();
        }
    ...
    rest() {
        if (window.player.gold >= 10) {
            if (window.player.hp === window.player.maxHp && window.player.mp === window.player.maxMp) return window.ui.log("Você já está totalmente descansado.");
            window.player.gold -= 10; window.player.hp = window.player.maxHp; window.player.mp = window.player.maxMp;
            window.ui.log(`🛌 Você pagou a estalagem e dormiu bem. Vida e Mana restaurados!`, "loot"); window.ui.update();
            this.saveGame();
        } else { window.ui.log("❌ O estalajadeiro o recusa. Ouro insuficiente (10💰)."); }
    }
};
...
window.combat = {
    ...
    start(enemyData, isBoss = false, bossLevel = 0) {
        ...
        window.ui.log(`⚠️ <b>${this.enemy.name} (Nvl ${this.enemy.lvl})</b> bloqueia o caminho!`, "dmg-taken");
    ...
    attack() {
        ...
        this.processPlayerDamage(damage, `⚔️ O seu ataque causou <b>${damage}</b> de dano!`);
    },
    ...
    useSkill() {
        ...
        if (window.player.mp < cls.skillCost) return window.ui.log(`❌ Sem Mana suficiente para ${cls.skillName}`, "dmg-taken");
        ...
        this.processPlayerDamage(damage, `✨ <b>${cls.skillName}</b> atingiu criticamente por <b>${damage}</b> de dano!`, "skill");
    },
    ...
    enemyTurn() {
        ...
        window.ui.log(`🩸 A criatura ataca: <b>${finalDamage}</b> de dano (Sua armadura absorveu ${window.player.def}🛡️).`, "dmg-taken");
    ...
    flee() {
        if (this.isBossFight) return window.ui.log("❌ VOCÊ NÃO PODE FUGIR DO DESTINO! (Luta de Boss)", "dmg-taken");

        if (Math.random() > 0.4) { window.ui.log("🏃 Você fugiu covardemente para as sombras."); this.end(); } 
        else { window.ui.log("🏃 O inimigo bloqueou a sua fuga!", "dmg-taken"); document.getElementById('combat-actions').style.pointerEvents = 'none'; setTimeout(() => this.enemyTurn(), 800); }
    },

    win() {
        window.ui.log(`🏆 <b>Vitória!</b> Recebeu ${this.enemy.gold}💰 e ${this.enemy.xp} EXP.`, "loot");
        window.player.gold += this.enemy.gold; 
        window.player.gainXp(this.enemy.xp); 

        if (this.isBossFight) {
            window.player.highestBossDefeated = this.isBossFight;
            window.ui.log(`👑 <b>CHEFÃO DERROTADO!</b> Você superou as trevas do nível ${this.isBossFight}.`, "skill");
            
            if (this.isBossFight === 50) {
                setTimeout(() => { alert("🎉 AETHELGARD ESTÁ SALVA! Você derrotou o Dragão Negro Primordial e zerou o jogo. A sua Lenda viverá para sempre!"); location.reload(); }, 3000);
                return;
            }
        }
        window.game.saveGame();
    ...
    lose() {
        window.ui.log(`💀 <b>A MORTE O ALCANÇOU...</b> O seu progresso será perdido.`, "dmg-taken");
        document.getElementById('combat-actions').style.display = 'none';
        localStorage.removeItem('aethelgard_save'); // Permadeath: O jogo apaga seu save se você morrer
...
window.ui = {
    ...
    updateInventoryModal() {
        ...
        if (window.player.equipped.weapon) {
            eqHTML += `<li class="inv-item"><span>🗡️ ${window.player.equipped.weapon.name} <span style="color:#ffb300;">(+${window.player.equipped.weapon.stat} Atk)</span></span> <button class="inv-btn" onclick="window.player.unequip('weapon')">Tirar</button></li>`;
        } else { eqHTML += `<li class="inv-item" style="color:#777;">🗡️ Arma Principal: (Vazio)</li>`; }
        
        if (window.player.equipped.offhand) {
            let sType = window.player.equipped.offhand.adds === 'atk' ? 'Atk' : 'Def';
            let color = window.player.equipped.offhand.adds === 'atk' ? '#ffb300' : '#90caf9';
            eqHTML += `<li class="inv-item"><span>🎒 ${window.player.equipped.offhand.name} <span style="color:${color};">(+${window.player.equipped.offhand.stat} ${sType})</span></span> <button class="inv-btn" onclick="window.player.unequip('offhand')">Tirar</button></li>`;
        } else { eqHTML += `<li class="inv-item" style="color:#777;">🎒 Mão Secundária: (Vazio)</li>`; }

        if (window.player.equipped.armor) {
            eqHTML += `<li class="inv-item"><span>🛡️ ${window.player.equipped.armor.name} <span style="color:#90caf9;">(+${window.player.equipped.armor.stat} Def)</span></span> <button class="inv-btn" onclick="window.player.unequip('armor')">Tirar</button></li>`;
        } else { eqHTML += `<li class="inv-item" style="color:#777;">🛡️ Armadura: (Vazio)</li>`; }
        ...
        if (window.player.inventory.length === 0) {
            bagHTML += "<li class='inv-item' style='text-align:center; color:#777;'>Nenhum equipamento solto.</li>";
        } else {
            window.player.inventory.forEach((item, index) => {
                ...
                let color = statType === 'Atk' ? '#ffb300' : '#90caf9';
                bagHTML += `<li class="inv-item"><span>${icon} ${item.name} <span style="color:${color};">(+${item.stat} ${statType})</span></span> <button class="inv-btn" onclick="window.player.equip(${index})">Equipar</button></li>`;
            });
        }
...
// AUTO-LOAD: Quando a tela carregar, tenta recuperar o save
